...
```json
{
  ...
  "error": {
    ...
    "message": "入参校验失败",
    "details": [
      { "path": ["0", "stage"], "message": "Invalid enum value", "code": "invalid_enum_value" }
    ]
  }
}
```
...
|---|---|---|---|
| 银行流水(收入) | `in` | `counterparty` | 直接 `settled`(occurredAt = settledAt) |
...
| `occurredAt` | `YYYY-MM-DD` | ✓ | 业务发生日(权责口径) |
...
> **重要**:`occurredAt`(权责) ≠ `settledAt`(现金) — 这是双视角财务的核心。
> 损益表按 `occurredAt` 汇总(看责任发生);现金流表按 `settledAt` 汇总(看真实资金流动)。务必分开维护。
...
| 数据类型 | 推荐频率 | stage 用法 |
|---|---|---|
| 银行流水 | 30 分钟 | 直接推 `settled`(occurredAt = settledAt) |
...
请求体使用 **camelCase**(`externalId`, `occurredAt`, `dueAt`, `settledAt`),响应也是 camelCase。

### 时间格式

- 日期字段:`YYYY-MM-DD`(如 `"2025-12-31"`)
