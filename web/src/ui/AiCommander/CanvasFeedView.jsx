...
const CanvasFeedView = ({ canvasId, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/35 backdrop-blur-[1px] flex items-end sm:items-center justify-center p-3">
      <div className="w-full max-w-5xl bg-[#f7f8fa] rounded-2xl border border-slate-100 shadow-2xl overflow-hidden">
        ...
        {loading ? (
          <div className="h-[85vh] md:h-[75vh] flex items-center justify-center">
            <Loader2 size={26} className="animate-spin text-slate-400" />
          </div>
        ) : articles.length === 0 ? (
          <div className="h-[85vh] md:h-[75vh] flex flex-col items-center justify-center text-slate-500">
            <ImageIcon size={24} className="text-slate-300 mb-2" />
            <p className="text-sm">该 Canvas 暂无文章</p>
          </div>
        ) : (
          <div className="flex flex-col md:grid md:grid-cols-[320px_1fr] h-[85vh] md:h-[75vh] min-h-0">
            <style>{`
              .canvas-hide-scrollbar::-webkit-scrollbar { display: none; }
              .canvas-hide-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
            `}</style>
            <div className="md:border-r border-b md:border-b-0 border-slate-100 overflow-x-auto md:overflow-y-auto p-3 flex md:flex-col gap-3 bg-white shrink-0 canvas-hide-scrollbar">
              {articles.map((a) => {
