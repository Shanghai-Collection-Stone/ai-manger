...
基础上下文模块：会话与消息的持久化与读取能力。任务型会话可保存 `workspaceId`，同一指定 sessionId 不允许跨工作区复用。
...
- **函数**:
  ...
  - `createSessionWithScope`: 创建租户/用户/类型/工作区范围会话，工作区冲突时生成新会话/create scoped workspace session
