...
- **函数**:
  ...
  - `buildDefaultSubagents`: 子代理配置。default 模式 6 个子代理: analysis_subagent(数据查询/分析 + 决策卡)、topic_orchestrate_subagent(文章)、frontend_subagent(图表/看板)、ops_subagent(**批量发布·内容分发·robot 发布执行**)、task_subagent(**任务编排:待办/工单 todo_* 全家桶**)、gallery_subagent(图组/图库)。**统一通过 `injectMiddleware` 给每个 SubAgent 注入 `agent.buildSubagentSanitizeMiddleware()`** —— deepagents 1.8.2 不会把主 agent customMiddleware 透传给 subagent 的内部 createAgent,出错时只看到 patchToolCallsMiddleware 抛 `expected AIMessage or Command, got object` 而没有原始数据,故必须显式注入到每个 SubAgent.middleware 字段/build default subagents with sanitize middleware injection
  - `resolveSubagentToolSets`: 把全量工具切成各专家工具集 —— analysis(数据源+搜索+决策卡)、topicOrchestrate、frontend、ops(robot_list+dashboard_config_patch+剩余 MCP,**不含 todo_**)、task(`todo_*` 全家桶)、gallery/resolve subagent tool sets
  ...
  - `getSupervisorPromptCN(envContext, currentActionSession?)`: 🆕 意图识别 LLM 系统提示词。**固定格式输出**: 强约束 LLM **整条回复只能是一个路由词**(image/article/data/frontend/publisher/task/chat),不带标点/解释/前后缀;**明确"只做分类不执行",严禁输出 tag-select/canvas-it 等代码块/卡片/工具调用,哪怕历史里有也不模仿**。归类规则: 闲聊/问指挥官/与 6 专家无关 → chat;6 类业务请求 → 对应专家词(data 含方案/决策/策略;publisher=批量发布,task=任务编排,两者区分);延续上轮任务 → 对应专家词/intent recognition fixed-format prompt
...
- **类型**:
  ...
  - `RecognizeIntentOptions`: recognizeIntent 入参 (systemPrompt + history 全部历史 + llm + currentActionSession)
  - `BuildExpertAgentOptions`: buildExpertAgent 入参 (route + experts + chatExpertPrompt + expertLLM + chatLLM + checkpointer)
- **函数(模块级)**:
  - `parseRouteToken(text)`: 从意图识别 LLM 纯文本回复硬解析路由 token(先 trim+lowercase 整体比对,再正则按单词边界取首个命中的 route 词)
  - `inferExpertByKeyword(text)`: 关键词规则推断专家(图组/文章/数据+决策/图表/批量发布/任务编排),LLM 没吐出可识别 token 时兜底
  - `isContinuationText(text)` / `extractMessageText(content)`: 延续口吻识别 / 从 string|多模态数组提取纯文本
- **类方法**:
  - `recognizeIntent(opts)`: 🆕 **第一步:意图识别**。一次独立的轻量 LLM `invoke([SystemMessage(prompt), ...全部历史])` —— 不是 tool 调用、不是 graph 节点。`parseRouteToken` 硬解析路由词;解析不到 → `inferExpertByKeyword` 关键词兜底 / 用户说"再来一组/我选定标签"+有 actionSession → 延续上轮专家;都没有 → 'chat'。**永远返回有效 RouteTarget,绝不抛错**/recognize intent via standalone llm call
