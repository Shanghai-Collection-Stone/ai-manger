...
- **函数**:
  ...
  - `resolveProviderDefaultBaseUrl`: 厂商默认baseUrl兜底（deepseek/nvidia/minimax/glm/kimi/shuyan；glm默认走z.ai Coding Plan入口/api/coding/paas/v4，kimi默认走 Moonshot OpenAI 兼容入口，shuyan(数眼智能)默认 https://platform.shuyanai.com/v1，备用节点 https://cloud.shuyanai.com/v1 需后台手填）/resolve provider default base url
  ...
  - `generateImageByRuntime`: 按默认提供商执行生图/图片编辑并返回图片。已对接 gemini / doubao(ark) / openai / shuyan。数眼智能走同步 `POST /v1/images/generations`，使用官方允许的 3:4 尺寸 `1728x2304`，兼容 `data[0].url` 与 `b64_json` 并立即持久化；当前官方通用图像文档只有文生图，带底图时明确拒绝，不会忽略底图静默降级。doubao/ark 文生图与图生图共用 /images/generations（无 /images/edits），图生图通过 body 的 image 字段传单字符串（URL 或 data:<mime>;base64,<b64>）；provider size 固定为 3:4 官方推荐像素 `1728x2304`。openai gpt-image-1/2：文生图 POST /images/generations、图生图 POST /images/edits（multipart, image=底图二进制，n=1）；provider size 不再跟随调用方输入，gpt-image-2 使用精确 3:4 `1536x2048`，gpt-image-1 使用其支持的竖版 `1024x1536`。**fetch 双层超时机制**: AbortSignal.timeout(10 分钟) + 实例级 `imageGenDispatcher` (undici Agent, headersTimeout/bodyTimeout=15 分钟) 覆盖 Node fetch 默认 undici 5 分钟 headersTimeout(否则 gpt-image 生图常在 5min 抛 `HeadersTimeoutError: UND_ERR_HEADERS_TIMEOUT`)；捕获 undici "fetch failed" 时把 error.cause 序列化(ConnectTimeoutError/SocketError/ENOTFOUND/CertificateError 等)落日志并抛 IMAGE_OPENAI_EDIT_NETWORK/IMAGE_OPENAI_GENERATE_NETWORK/generate image by configured runtime
