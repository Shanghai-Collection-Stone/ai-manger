...
export class AgentService {
   ...
   * 纯净无字底图/构图空间/点击率，标题由上层画布独立承载；内页强调"少文字、重内容"，禁止封面化包装。当
  ...
  private buildMeituEditPrompt(input: {
    ...
    const hardBlock = [
      ...
      '3. 文字限制:输出必须是纯净无字封面底图。上文的主标题/副标题只用于理解主题和预留构图空间，严禁把任何标题、副标题、标签文字、字母、数字、水印、平台 logo、网址或二维码画进图片；上层灵感画布会把文案作为独立可编辑文字图层叠加。',
      '4. 装饰元素:允许叠加少量无字装饰(光斑、彩带、箭头、星芒、胶带或简洁色块)提升封面活力与点击率；禁止漫画对话框、拟声词和任何包含文字的贴纸，装饰不得遮盖主体面部或后续文字留白区。',
