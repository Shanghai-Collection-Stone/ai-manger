...
- `controller/xhs-topic.dto.ts` — 生成层级、提示词、母选题、母题配图标签、数量与检索开关校验。
- `entities/xhs-topic.entity.ts` — 选题候选、数据库实体、母题配图标签、母子列表、生成输入、Todo 结果与接口响应类型。
- `services/xhs-topic-repository.service.ts` — MongoDB 索引、真实母子选题列表（已存入文章库的子题不再返回）、母题配图标签持久化、批量入库和级联删除。
- `services/xhs-article-generation.service.ts` — 文章 Todo、Agent 内存文章工具、母题配图约束、真实图库标签选择、Duck 搜索、生文图组工作流与文章落库。
...
- `PersistXhsTopicCandidateDto({ title, topicType, imageTags? })` — 校验用户确认入库的单条候选及母题固定配图标签 | keywords: 保存选题候选, 题目类型, 母题配图标签, persist-topic-candidate, topic-type, mother-image-tags
...
- `UpdateXhsTopicDto({ title?, topicType?, imageTags?, status? })` — 校验真实选题内容、状态或母题固定配图标签更新 | keywords: 更新真实选题, 选题状态, 母题配图标签, update-persisted-topic, topic-status, mother-image-tags
...
- `XhsTopicRepositoryService.update(id, input, scope)` — 更新标题、类型、发布状态或母题固定配图标签 | keywords: 更新选题, 发布状态, 母题配图标签, update-topic, publish-status, mother-image-tags
...
- `XhsTopicController.create(req, dto)` — 批量入库所选候选及母题配图标签并返回真实列表 | keywords: 保存真实选题, 批量创建, 母题配图标签, persist-selected-topics, bulk-create, mother-image-tags
- `XhsTopicController.remove(req, dto)` — 批量删除选题并级联子题 | keywords: 删除真实选题, 级联删除, delete-persisted-topics, cascade-delete
- `XhsTopicController.update(req, id, dto)` — 修改真实选题、状态或母题固定配图标签 | keywords: 更新真实选题, 发布状态, 母题配图标签, update-persisted-topic, publish-status, mother-image-tags
...
- `XhsTopicCandidate` — 包含 `title`、`topicType` 与可选母题 `imageTags` 的候选。
- `XhsTopicEntity` — MongoDB 中持久化的母题或子题；母题可保存最多五个 `imageTags`。
...
- `XhsTopicCreateInput` — 用户确认候选的批量入库输入，可随母题候选一次性保存 `imageTags`。
- `XhsTopicUpdateInput` — 真实选题更新输入；通过母题 `imageTags` 设置或清空统一配图标签。
...
`GET /api/xhs-topic` 从 `xhs_topics` 返回当前租户用户的母子选题工作台，并在每个母题分组原样返回 `imageTags`；同时通过 `articles.source=xhs-topic` 与 `meta.xhsTopicId` 过滤已经存入选题文章库的子题，历史已入库文章同样生效，库内文章删除后对应子题会重新出现。无租户账号同时兼容历史缺失字段与 MongoDB 序列化的 `null`；`POST /api/xhs-topic` 将用户确认的候选及母题 `imageTags` 批量入库并返回最新工作台；`PATCH /api/xhs-topic/:id` 更新标题、类型、状态或母题 `imageTags`，传空数组即可恢复原配图链路；`DELETE /api/xhs-topic` 删除当前用户指定选题，母题命中时级联删除所有子题。入口分别声明 `read/create/update/delete XhsTopic` 权限。
