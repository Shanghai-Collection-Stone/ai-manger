...
- `agent:<24-hex-id>` — 新格式，读取后台 agent 配置后路由到对应模块（claw / super_claw_data_tracking 等）
...
| `handleSuperClawDataTracking(todo,agentCtx)`           | 为 SuperClaw 数据抓取任务签发专用 Token 并通知平台主动推送 | keywords: SuperClaw任务待领, 签发任务令牌, super-claw-task-pending, issue-task-token |
| `notifySuperClawTask(tenantId?,workspaceId?)`          | 按租户绑定或平台工作区固定节点触发主动推送               | keywords: 通知SuperClaw任务, 主动推送触发, notify-super-claw-task, active-push-trigger |
...
| `SUPER_CLAW_DATA_TRACKING_AGENT_MODULE` | SuperClaw 专用数据抓取 Agent 模块标识                           | keywords: SuperClaw数据抓取, 专用代理模块, super-claw-data-tracking, dedicated-agent-module |
...
- **SuperClawTaskChannelService** (`super-claw` 模块) — Todo 入队后通过平台维护的在线通道主动推送
