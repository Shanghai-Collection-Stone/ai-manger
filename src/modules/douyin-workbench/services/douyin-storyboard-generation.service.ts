...
export class DouyinStoryboardGenerationService {
  ...
  /**
   * @description 为当前用户子选题生成四至十二段真实分镜并持久化；生成前从租户图库挑出相关候选图交给 LLM 逐段选图，
   *   LLM 没选的镜头按相关度自动补图。`onProgress` 在每写入一段时回报真实段数，供后台任务展示进度。
   * @keyword-cn 生成真实分镜, 保存镜头脚本, 分镜自动配图
   * @keyword-en generate-real-storyboard, persist-shot-script, storyboard-auto-image
   */
  async generate(
    topicId: number,
    ...
    const parent = topic.parentId
      ...
      : null;
    const candidates = await this.images.loadCandidates(
      [parent?.title, topic.title, requirement].filter(Boolean).join(' '),
      scope,
    );
    const shots: DouyinStoryboardShot[] = [];
    const addShot = this.createShotTool(shots, candidates, onProgress);
    const imagePrompt = buildImageCandidatePrompt(candidates);
    ...
    return { topic: saved, storyboard: shots };
  }
  ...
  private async runAgent(
    ...
    await this.agentService.runWithMessages({
      config: {
        system: [
          '你是抖音短视频导演。围绕给定子选题制作可直接拍摄或交给视频生成器执行的竖屏分镜。',
